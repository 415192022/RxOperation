...
public class FABRevealLayout extends RelativeLayout {
    ...
    private FreshDownloadView fab = null;
    ...
    public FABRevealLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        setExpandingViewColor(typedArray.getColor(R.styleable.FABRevealLayout_fab_color, Color.parseColor("#FF0000")));
    }
    ...
    private void setupView(View child) {
        if (child instanceof FreshDownloadView) {
            setupFAB(child);
...
//        }
    ...
    private int expandingViewColor = 0xFF000000;

    public void setExpandingViewColor(int color) {
        this.expandingViewColor = color;
    }

    private void prepareForReveal() {
        circularExpandingView.getLayoutParams().height = getMainView().getHeight();
        circularExpandingView.setColor(expandingViewColor);
