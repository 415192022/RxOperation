package com.li.utils.ui.smoothcheckbox;
...
public class SmoothCheckBox extends View implements Checkable {
    ...
    private static final int COLOR_FLOOR_UNCHECKED = Color.parseColor("#427fed");
    ;
...
//        Drawable bitmapDrawable=ta.getDrawable(R.styleable.SmoothCheckBox_drawable_bitmap);
//        bitmap= CommonUtils.drawableToBitamp(bitmapDrawable);
    ...
    private void drawCenter(Canvas canvas) {
        ...
        if (null != bitmap) {
            canvas.drawBitmap(small(bitmap, 0.4f, 0.4f), mCenterPoint.x - bitmap.getWidth() / 5, mCenterPoint.y - bitmap.getHeight() / 5, mPaint);
        }
        if (null !=str &&!str.equals("")  ) {
            clearImageBitmap();
            mPaint.setColor(Color.parseColor("#FFFFFF"));
            mPaint.setTextSize(CompatUtils.dp2px(context, 13f));
            mPaint.setStrokeWidth(100);
            Rect rect =new Rect();
            //获得字体矩形高度
            mPaint.getTextBounds(str,0,str.length(),rect);
            canvas.drawText(str, mCenterPoint.x - (mPaint.measureText(str) / 2), mCenterPoint.y+(rect.height()/2), mPaint);
        }
    }
    ...
    public void setImageBitmapTrans(){
        if(null != bitmap){
