package com.li.utils.ui.progressbar;
...
public class FreshDownloadView extends View {
    ...
    //    private final int top = 0 + getPaddingTop();
//    private final int bottom = getHeight() - getPaddingBottom();
//    private final int left = 0 + getPaddingLeft();
//    private final int right = getWidth() - getPaddingRight();
    @Override
    ...
    private void updateArrow() {
        ...
        path2.lineTo((float) (mRealLeft + radius - Math.sin(Math.toRadians(40))
                * radius
                * 0.46f
        ), mArrowStart + radius - radius * .46f);

        //箭头右边部分
        path3.moveTo(mRealLeft + radius, mArrowStart + radius);
        path3.lineTo((float) (mRealLeft + radius + Math.sin(Math.toRadians(40))
                * radius
                * 0.46f
        ), mArrowStart + radius - radius * .46f);
    ...
    public void setRadius(float radius, int t, int b, int l, int r) {
        if (radius < 20) {
            radius = 20;
        }
        this.radius = radius;
        updateArrow();
        updateBounds(t, b, l, r);
        invalidate();
    }
